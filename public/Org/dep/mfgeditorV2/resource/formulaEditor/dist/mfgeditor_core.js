    ...
    var n = e.editorCanvas = {
        ...
        showfomular: function (e, t) {
            ...
            i.each(function () {
                ...
                if (i != null) {
                    i = i.replace(/\\\(/gi, "<frm>").replace(/\\\)/gi, "</frm>");
                    var o = i.match(r);
                    if (o != null) {
...
editorCanvas.simbol.prototype.options["rightarrow"] = function () {
    var e = new editorCanvas.operation;
    e.regroup = function (e, t) {
        editorCanvas.s.regroup(e.child[0], t);
        e.width = e.child[0].width;
...
editorCanvas.simbol.prototype.options["equation3"] = function () {
    ...
    e.regroup = function (e, t) {
        ...
        e.value = "\\begin{cases}{" + e.child[0].getValue() + "}\\\\{" + e.child[1].getValue() + "}\\\\{" + e.child[2].getValue() + "}\\end{cases}";
        e.size = t;
        e.offset = (e.height - t) / 2
    }
    ;
...
editorCanvas.simbol.prototype.options["underset"] = function () {
    ...
    e.regroup = function (e, t) {
        ...
        e.level = (e.child[0].level > e.child[1].level ? e.child[0].level : e.child[1].level) + 1;
        e.size = t;
        e.value = "\\underset{" + e.child[0].getValue() + "}{" + e.child[1].getValue() + "}";
    ...
    e.draw = function (e, t, n) {
        ...
        editorCanvas.s.transaction(e.child[0], t + (e.width - e.child[0].width) / 2, n + e.child[1].height*0.5)
    }
    ;
    e.children = 2;
    for (var t = 0; t < e.children; t++)
...
editorCanvas.chemistryRegroup = function (e, t, n, r) {
    ...
    if (r == 1) {
        // {\frac{\underline{高温}}{饭}}
        e.value = "{\\frac{\\underline{" + e.child[0].value + "}}{" + e.child[1].value + "}}";
        // e.value = "{\\twolinefrac{" + e.child[0].value + "}{" + e.child[1].value + "}}"
    } else if (r == 2) {
        //\ce{2H2 + O2 ->T[燃烧] 2H2O}
        e.value = "{\\ce{->T[" + e.child[0].value + "][" + e.child[1].value + "]}}"
...
;
editorCanvas.chemistryRegroup1 = function (e, t, n, r) {
    editorCanvas.s.regroup(e.child[0], t * .5);
    editorCanvas.s.regroup(e.child[1], t * .5);
    if (n == 1) {
        e.level = e.child[0].level + 1;
...
;
editorCanvas.chemistryDraw1 = function (e, t, n, r, i) {
    if (r == 1) {
        editorCanvas.s.transaction(e.child[0], t + (e.width - e.child[0].width) / 2, n + e.size * .4)
    } else if (r == 2) {
        editorCanvas.s.transaction(e.child[0], t + (e.width - e.child[0].width) / 2, n + e.size * .4);
...
;
editorCanvas.Analysis.prototype.checkout1 = function () {
...
editorCanvas.Analysis.prototype.checkout = function () {
    ...
    //针对花括号和正宗latex不匹配问题
    if(this.latex.indexOf("\\begin{cases}")>-1&&this.latex.indexOf("\\end{cases}"))
    {
        this.latex=this.latex.replace("\\begin{cases}","\\left\\{{\\begin{array}{l}").replace("\\end{cases}","\\end{array}}\\right");
    }
    //针对化学公式和正宗latex不匹配问题
    // var pvalue='\\({\\frac{\\underline{高温}}{}}\\)\\({\\frac{\\underline{太阳能}}{催化剂}}\\)\\({\\ce{<=>T[1kmjkjlkhnjkj][2]}}\\)\\({\\ce{->T[MnO2][]}}\\)';
    // e.value = "{\\frac{\\underline{" + e.child[0].value + "}}{" + e.child[1].value + "}}";
    // e.value = "{\\twolinefrac{" + e.child[0].value + "}{" + e.child[1].value + "}}"
    // e.value = "{\\ce{->T[" + e.child[0].value + "][" + e.child[1].value + "]}}"
    // e.value = "{\\rowfrac{" + e.child[0].value + "}{" + e.child[1].value + "}}"
    ...
    var twolinefracReg=/\\frac{\\underline{(.*?)}}{(.*?)}/gi;
    var rowfracReg=/\\ce{-(>|&gt;)T\[(.*?)\]\[(.*?)\]}/gi;
    var twoarrowfrac=/\\ce{(<|&lt;)=(>|&gt;)T\[(.*?)\]\[(.*?)\]}/gi;
    if(twolinefracReg.test(this.latex))
    {
        this.latex = this.latex.replace(twolinefracReg, "\\twolinefrac{$1}{$2}");
    }
    if(rowfracReg.test(this.latex))
    {
        this.latex = this.latex.replace(rowfracReg, "\\rowfrac{$2}{$3}");
    }
    if(twoarrowfrac.test(this.latex))
    {
        this.latex = this.latex.replace(twoarrowfrac, "\\twoarrowfrac{$3}{$4}");
    }
    this.latex = this.latex.replace(/(\\[a-zA-Z]*)\s([a-zA-Z])/g, "$1{$2}").replace(/\s|\n/g, "").replace(/<em>(array|left|right|begin|end)<\/em>/gi, "$1").replace(/({)<em>(.*?)<\/em>(.)/gi, function (e, t, n, r) {
        if (r == "}") {
            return t + "\\" + "em{" + n + "}" + r
...
;
editorCanvas.Analysis.prototype.options["rightarrow"] = function (e) {
    return editorCanvas.a.unary(e, "rightarrow")
}
;
...
(function (e) {
    ...
    e.outplane = {
        ...
        popList: function (t) {
            ...
            if (this.currentId > 0) {
                ...
                if (s == this.currentId) {
                    //判断是否是第一行
                    if (r.closest(".qaEditor").find("tr:first")[0] == r[0]) {
                        return
                    }
    ...
    e.paste = function () {
        ...
        if (!e.outplane.ispaste) {
            ...
            i.append(s);
            if (document.selection) {
                document.getElementById("2236").values = window.clipboardData.getData("Text")
            } else {
                $("#2236").focus();
                $("#2236").select()
            }
                        ...
                        if ($(n).parents(".inserttablerow").length == 0) {
                            ...
                            $(n).find("td").each(function (index, item) {
                                if (item.id) {
                                    ...
                                    h.find("#" + item.id).parent().closest("td").append(v);
                                }
                            });
                            h.find("input").remove();
                            h.find("td").each(function (index, item) {
                                if (item.id) {
    ...
    e.getValueFromTd = function (n) {
        ...
        if ($(n).attr("edittype") == "Text") {
            ...
            s += "\\(" + e.reRender($(n).find("canvas").data("latex")) + "\\)";
        } else if ($(n).attr("edittype") == "Image") {
            s += $(n).html();
        }
        return s;
    ...
    e.outplane.valueToCellPro1 = function (n, r, i) {
        var s = e;
        var o = /<frm>(.*?)<\/frm>/gi;
        var u = /<femt>(.*?)<\/femt>/gi;
        var a = /<ftxt>(.*?)<\/ftxt>/gi;
    ...
    e.outplane.valueToCellPro = function (n, r, i) {
        n = n.replace(/\\\(/gi, "<frm>").replace(/\\\)/gi, "</frm>");
        var s = e;
        var o = /<frm>(.*?)<\/frm>/gi;
        //var o = /\\\((.*?)\\\)/gi;
        var u = /<femt>(.*?)<\/femt>/gi;
        var s1 = /<fspan>(.*?)<\/fspan>/gi;
        var a = /<ftxt>(.*?)<\/ftxt>/gi;
        var f = /<ftxt>(.*?)<\/ftxt>|<frm>(.*?)<\/frm>|<femt>(.*?)<\/femt>|<fspan>(.*?)<\/fspan>/gi;
        var l = /(<img.*?(?:>|\/>))/gi;
        var span = /(<span.*?>(.*?)<\/span>)/gi;
        //TODO table
        var table = /(<table>(.*?)<\/table>)/gi;
        var c = /(^|<\/frm>|<\/femt>|<\/fspan>)((.|\n)*?)(?=<frm>|<femt>|<fspan>|$)/gi;
        var h = [];
        if (i) {
            h = t(n)
        } else {
        ...
        for (var p = 0; p < h.length; p++) {
            ...
            n = h[p];
            var d = n.replace(l, "<femt>$1</femt>");
            var sd = d.replace(span, "<fspan>$1</fspan>");
            n = sd.replace(c, function (e, t, n) {
                return t + "<ftxt>" + n + "</ftxt>";
            });
            var v = r;
            if (!v[0]) {
            ...
            for (var g = 0; g < m.length; g++) {
                if (a.test(m[g])) {
                    ...
                    s1.lastIndex = 0;
                    var b = m[g].replace(s1, "$1");

                    var styleContent = "";
                    if (m[g].indexOf("text-decoration") > -1) {
                        styleContent += editorCanvas.styles.UnderLine + ",";
                    }
                    var breg = /<b>(.*?)<\/b>/gi;
                    var emreg = /<em>(.*?)<\/em>/gi;
                    var delreg = /<del>(.*?)<\/del>/gi;
                    if (breg.test(b)) {
                        styleContent += editorCanvas.styles.Bold + ",";
                    }
                    if (emreg.test(b)) {
                        styleContent += editorCanvas.styles.Itaily + ",";
                    }
                    if (delreg.test(b)) {
                        styleContent += editorCanvas.styles.ThroughLine + ",";
                    }
                    this.id++;
                    this.list.push(this.id);
                    var y = '<td id="textcell' + this.id + '" edittype="Text" nowrap="nowrap" editstate="true" style="vertical-align: bottom;">' + '<input type="text" class="celltext" style="width: 10px;" onpaste="editorCanvas.paste()">' + "</td>";
                    v.append(y);
                    $($("#textcell" + this.id).find("input")[0]).attr("styleContent", styleContent);
                    $("#textcell" + this.id).find("input")[0].value += b.replace(span, "$2").replace(breg, "$1").replace(emreg, "$1").replace(delreg, "$1");
                    this.repairInput($("#textcell" + this.id).find("input"));
                    this.reWriteCssInput($("#textcell" + this.id).find("input"));
                    this.currentId++;
    ...
    ;
    e.activeFormulaZone = function (t, n,rows) {
        t = e.filter(t);
